...
export class Populate {
    ...
    async _addObject() {
        ...
        let entityClasses = [];
        for (let superEntityClass in interfaceInstance.objectViews) {
            for (let entityClass in interfaceInstance.objectViews[superEntityClass].objects) {
                entityClasses.push({[textM.fields.description]: `${superEntityClass}.${entityClass}`});
            }
        }
    ...
    async _addObjectActions(objects) {
        ...
        for (let object of objects) {
            ...
            let objecDescriptionPath = object.fields.description.split(".");
            for (let method in interfaceInstance.objectViews[objecDescriptionPath[0]].objects[objecDescriptionPath[1]].methods) {
                actionsArray.push({[textM.fields.description]: method, [textM.fields.objectId]: objectID})
            }
        }
    ...
    async _addObjectsFields(objects) {
        ...
        let actionField = EntityParserUtil.parse(await objView.query(textM.role.objects_fields, textM.dbMethod.get, {}));
        ...
        for (let object of objects) {
            let objectID = object.fields.ID;
            let objecDescriptionPath = object.fields.description.split(".");
            for (let method in interfaceInstance.objectViews[objecDescriptionPath[0]].objects[objecDescriptionPath[1]].fields) {
                fieldsArray.push({[textM.fields.description]: method, [textM.fields.objectId]: objectID})
            }
        }
            ...
            for (let filter of filters) {
                switch (role.fields.description) {
                    case textM.rang.admin:
                        //У популированного админа есть суперфильтр со всеми действиями, поэтому некоторые фильтры отменяем у админадля тестов суммы interfaceInstance и фильтров по полям.
    ...
    async _addUsers() {
        ...
        insertedUsersArray.push(this._getUser("userAdmin", "admin@mail.ru", "password", "adminName"));
        insertedUsersArray.push(this._getUser("userUser", "user@mail.ru", "password", "userName"));
        insertedUsersArray.push(this._getUser("userReadAndDeleteNotUsers", "read@mail.ru", "password", "readName"));
    ...
    async _addUsersRoles(users, roles) {
        ...
        for (let user of users) {
            for (let role of roles) {
                if (user.fields.description === "userAdmin" && role.fields.description === textM.rang.admin) {
                    userRoleArray.push(this._getUserRole(user, role));
                }
                if (user.fields.description === "userUser" && role.fields.description === textM.rang.user) {
                    userRoleArray.push(this._getUserRole(user, role));
                }
                if (user.fields.description === "userReadAndDeleteNotUsers" && role.fields.description === textM.rang.readOnly) {
                    userRoleArray.push(this._getUserRole(user, role));
                }
            }
        }
