...
export class PopulateRinTest {
    ...
    async _addProduct() {
        ...
        let insertedParent = await this.objView.query(textM.supply.product, textM.dbMethod.insert, {
        ...
        for (let hierarchyNumber = 0; hierarchyNumber < quantityHierarchy; hierarchyNumber++) {
            ...
            for (let productNumber = 0; productNumber < quantityProductInHierarchy; productNumber++) {
                ...
                let insertedObjects = await this.objView.query(textM.supply.product, textM.dbMethod.insert, {
                ...
                for (const objectField of objectsFields) {
                    for (const sign of signs) {
                        for (const product of products) {
                            if (object.fields.description === textM.supply.query_position &&
                                objectField.fields.objectID === object.fields.ID && objectField.fields.description === "productID" &&
                                sign.fields.sign === textM.sign.rin && product.fields.description === "h-0 pr-1" &&
                                filter.fields.description === "rin") {
                                filtersFieldsArray.push(EntityBuilder.buildFilterFields(object, objectField, filter, sign, product.fields.ID));
                            }
                        }
                    }
                }
                    ...
                    for (const sign of signs) {
                        for (const query of queries) {
                            if (object.fields.description === textM.supply.query &&
                                objectField.fields.objectID === object.fields.ID && objectField.fields.description === "ID" &&
                                sign.fields.sign === textM.sign.equal && query.fields.description === "z1" &&
                                filter.fields.description === "rin") {
                                filtersFieldsArray.push(EntityBuilder.buildFilterFields(object, objectField, filter, sign, query.fields.ID));
                            }
                        }
                    }
                }
